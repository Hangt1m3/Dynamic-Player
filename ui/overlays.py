...
class NotificationWidget(QWidget):
    ...
    def show_notification(self, anim_type, direction, screen_pos, duration=4000, anim_duration=500, permanent=False):
        ...
        # Disconnect any previous connections to avoid double calls or wrong state
        ...
        # Connect the finished signal only to the relevant animation
        if main_anim:
            main_anim.finished.connect(self._on_anim_finished)

        self.show()
        # The timer is NOT started here; it starts in _on_anim_finished after entrance completes.

    def fade_out(self):
        self._is_fading_out = True; self.opacity_anim.stop(); self.pos_anim.stop(); self.timer.stop()
        ...
        if self._anim_type == "Fade":
            ...
            self.pos_anim.setStartValue(self.content_widget.pos()); self.pos_anim.setEndValue(target_pos); self.pos_anim.setDuration(self._anim_duration); 
            self.pos_anim.setEasingCurve(QEasingCurve.InExpo); self.pos_anim.start()
            main_anim = self.pos_anim
